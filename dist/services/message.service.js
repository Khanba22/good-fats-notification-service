...
exports.buildMessageForEvent = buildMessageForEvent;
exports.isEventEnabled = isEventEnabled;
exports.getConfiguredEvents = getConfiguredEvents;
exports.getEnabledEvents = getEnabledEvents;
 ...
 */
function buildMessageForEvent(topic, payload) {
    ...
    try {
        const message = (0, template_service_1.renderTemplate)(entry.template, payload);
 ...
 */
function isEventEnabled(topic) {
    const templates = loadTemplates();
    const entry = templates[topic];
    if (!entry) {
        console.warn(`[MessageService] No template for topic "${topic}". Config has ${Object.keys(templates).length} keys. Loaded from: ${TEMPLATES_PATH}`);
        return false;
    }
